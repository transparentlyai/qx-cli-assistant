...
DEFAULT_PROHIBITED_SHELL_PATTERNS: List[str] = [
    ...
    "mv /* /dev/null", "wget * | bash", "curl * | bash", "wget * | sh", "curl * | sh",
    "*(){ :;};*", "echo * > /etc/passwd", "echo * > /etc/shadow", "rm -rf ~", "rm -rf .",
    "find / -delete", "find . -delete", "shred * /dev/*", "wipe /dev/*",
    "fdisk /dev/*", "parted /dev/*", "gparted /dev/*", "userdel root", "groupdel root",
    "passwd -d root", "passwd -l root", "chown root* /*", "chown -R * /*",
    "chattr -i *", "chattr +i /*", "reboot", "shutdown *", "halt", "poweroff",
    "init 0", "init 6", "iptables -F", "iptables -X", "iptables -P INPUT DROP",
    "iptables -P FORWARD DROP", "iptables -P OUTPUT DROP", "ufw disable",
    "systemctl stop firewalld", "systemctl disable firewalld", "insmod *", "rmmod *",
    "modprobe *", "sysctl *", "echo * > /proc/sys/*", "* | base64 -d | bash",
    "* | base64 -d | sh", "history -c", "echo > ~/.bash_history", "kill -9 1",
    "pkill init", "mount -o remount,ro /", "mount /dev/sd* /mnt; rm -rf /mnt/*",
]
DEFAULT_AUTO_APPROVED_SHELL_PATTERNS: List[str] = [
    "ls", "ls *", "cd", "cd *", "pwd", "echo", "echo *", "cat", "cat *", "head", "head *",
    "tail", "tail *", "grep", "grep *", "find", "find *", "mkdir", "mkdir *", "touch",
    "touch *", "cp", "cp *", "mv", "mv *", "python --version", "python -m *",
    "python3 --version", "python3 -m *", "pip list", "pip show *", "pip freeze",
    "uv list", "uv pip list", "uv pip show *", "uv pip freeze", "cloc", "cloc *",
    "git", "git *", "date", "cal", "uptime", "whoami", "id", "uname", "uname *", "df",
    "df *", "du", "du *", "wc", "wc *", "less", "less *", "more", "more *", "diff",
    "diff *", "comm", "comm *", "sort", "sort *", "uniq", "uniq *", "ps", "ps *", "top",
    "env", "printenv", "printenv *", "export -p", "man", "man *", "info", "info *",
    "tldr", "tldr *", "* --help", "stat", "stat *", "which", "which *", "whereis",
    "whereis *", "type", "type *", "history", "clear", "ping", "ping *", "ip", "ip *",
    "ifconfig", "ifconfig *", "netstat", "netstat *", "ss", "ss *", "host", "host *",
    "dig", "dig *", "nslookup", "nslookup *", "tar -tvf *", "zipinfo *", "unzip -l *",
    "gzip -l *", "gunzip -l *", "zcat *", "bzcat *", "xzcat *", "basename *",
    "dirname *", "readlink *", "test *", "[ * ]", "alias", "jobs", "file", "file *",
]
...
def execute_shell_tool(
    ...
    if _is_command_prohibited(command_to_consider):
        ...
        logger.error(err_msg)
        console.print(f"[error]Command prohibited by policy:[/error] '{command_to_consider}'")
    ...
    if needs_confirmation:
        ...
        if decision_status == "modified" and final_value is not None:
            ...
            if not command_to_execute:
                ...
                logger.warning(f"Original command '{command_to_consider}' modified to empty.")
                console.print(f"[error]{err_msg}[/error] Original: '{command_to_consider}'")
                return ExecuteShellPluginOutput(command=command_to_consider, error=err_msg)
    ...
    try:
        ...
        console.print(f"[error]Failed to execute command '{command_to_execute}':[/error] {e}")
        return ExecuteShellPluginOutput(command=command_to_execute, error=err_msg)
...
if __name__ == "__main__":
    ...
    input1 = ExecuteShellPluginInput(command="echo 'auto hello'") # Using echo for predictable output
    output1 = execute_shell_tool(dummy_ctx, input1)
    test_console.print(f"Output 1 STDOUT: {output1.stdout}")
    assert output1.stdout == "auto hello" and output1.return_code == 0 and output1.error is None
    ...
    assert output2.stdout is None and output2.stderr is None and output2.return_code is None
    ...
    assert output3.stdout == "Hello QX Shell" and output3.return_code == 0 and output3.error is None
    ...
    assert output4.stdout is None and output4.stderr is None and output4.return_code is None
    ...
    test_console.print(f"Please respond 'm', then enter '{modified_cmd}', then 'y' if asked to confirm modified.")
    ...
    test_console.print("Please respond 'm', then enter an empty string (just press Enter).")
    ...
    assert output7.command == "echo 'this will be modified to empty'" # Original command reported
    ...
    input8 = ExecuteShellPluginInput(command="echo 'this will be modified to sudo false'")
    ...
    assert output8.error == "Error: Modified command 'sudo false' is prohibited by policy."
    ...
    output9 = execute_shell_tool(dummy_ctx, input9)
