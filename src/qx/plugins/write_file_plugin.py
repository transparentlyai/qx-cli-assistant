...
def _generate_write_preview(
    ...
    if file_path.exists() and file_path.is_file():
        try:
            ...
            diff_lines = list(difflib.unified_diff(
                old_lines, new_lines,
                fromfile=f"Current: {file_path_str}", tofile=f"New: {file_path_str}",
                lineterm="\n", n=3
            ))
            if diff_lines:
                return Syntax("".join(diff_lines), "diff", theme="vim", line_numbers=False, word_wrap=True)
            return Text("[bold yellow]No textual changes detected in diff (content might be identical after normalization).[/bold yellow]")
        except Exception as e:
            logger.error(f"Error generating diff for {file_path_str}: {e}", exc_info=True)
    ...
    if line_count > MAX_PREVIEW_LINES:
        ...
        return Syntax(display_content_str, lexer_name, theme=syntax_theme, line_numbers=True, word_wrap=True, background_color=bg_color)
    else:
        return Syntax(new_content, lexer_name, theme=syntax_theme, line_numbers=True, word_wrap=True, background_color=bg_color)

# --- End of _generate_write_preview ---

class WriteFilePluginInput(BaseModel):
    """Input model for the WriteFilePluginTool."""
    path: str = Field(..., description="Path to the file. Parent dirs created if needed.")
    content: str = Field(..., description="Content to write.")

class WriteFilePluginOutput(BaseModel):
    """Output model for the WriteFilePluginTool."""
    path: str = Field(description="The (expanded and possibly modified) path of the file.")
...
def write_file_tool(
    ...
    args: WriteFilePluginInput
) -> WriteFilePluginOutput:
    """
    PydanticAI Tool to write content to a file.
    Requires user confirmation. Allows path modification.
    Restricted by policy to project or user's home directory.
    """
    console = ctx.deps.console
    original_path_arg = args.path
    path_to_consider = os.path.expanduser(original_path_arg) # Start with this for checks and prompts
    ...
    if not absolute_path_to_evaluate.is_absolute():
        absolute_path_to_evaluate = current_working_dir.joinpath(path_to_consider).resolve()
    else:
    ...
    if not is_path_allowed(absolute_path_to_evaluate, project_root, USER_HOME_DIR):
        ...
        logger.error(f"Write access denied by policy (plugin pre-check) for path: {path_to_consider}")
        return WriteFilePluginOutput(path=path_to_consider, success=False, message=err_msg)

    # Generate preview
    # TODO: Get syntax_theme from config or context if made configurable
    preview_renderable = _generate_write_preview(path_to_consider, args.content, syntax_theme="vim")
    ...
    if decision_status == "modified" and final_value is not None:
        ...
        logger.info(f"Write path modified by user from '{path_to_consider}' to '{path_to_write}'.")
        ...
        if not absolute_modified_path.is_absolute():
            absolute_modified_path = current_working_dir.joinpath(path_to_write).resolve()
        else:
...
if __name__ == "__main__":
    ...
    input1 = WriteFilePluginInput(path="new_project_file.txt", content="Hello from write plugin!")
    ...
    if output1.success:
        assert (test_project / "new_project_file.txt").read_text() == "Hello from write plugin!"
    ...
    input2 = WriteFilePluginInput(path="existing_project_file.txt", content="New line 1\nOld line 2\nNew line 3")
    test_console.print("Please respond 'y' to the prompt.")
    output2 = write_file_tool(dummy_ctx, input2)
    test_console.print(f"Output 2: {output2}")
    if output2.success:
         assert (test_project / "existing_project_file.txt").read_text() == "New line 1\nOld line 2\nNew line 3"
    ...
    os.chdir(original_cwd) # Change CWD to ensure tilde path is outside project for confirmation
    ...
    input3 = WriteFilePluginInput(path=f"~/{home_test_file_original_name}", content="Content for modified path.")
    test_console.print(f"At the prompt: respond 'm', then enter '~/{home_test_file_modified_name}', then 'y' (if a second confirm appears) or just enter.")
    ...
    if output3.success:
        ...
        assert (USER_HOME_DIR / home_test_file_modified_name).read_text() == "Content for modified path."
        (USER_HOME_DIR / home_test_file_modified_name).unlink(missing_ok=True)
    (USER_HOME_DIR / home_test_file_original_name).unlink(missing_ok=True) # cleanup original if created by mistake

    # Test 4: Denied by policy
    test_console.print("\n[bold]Test 4: Write to /etc/somefile (policy denial)[/]")
    input4 = WriteFilePluginInput(path="/etc/this_should_fail.txt", content="Forbidden content")
    ...
    input5 = WriteFilePluginInput(path="user_denies_this.txt", content="This won't be written.")
