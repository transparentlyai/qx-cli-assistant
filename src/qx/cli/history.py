...
class QXHistory(History):  # Inherit from History
    ...
    def __init__(self, history_file_path: Path = QX_HISTORY_FILE):
        ...
        logger.debug(f"History file path resolved to: {self.history_file_path.absolute()}")
        ...
        logger.debug(f"Parent directory exists: {self.history_file_path.parent.exists()}")
    ...
    def load_history(self):
        ...
        try:
            ...
            logger.error(f"File permissions: {oct(self.history_file_path.stat().st_mode)[-3:] if self.history_file_path.exists() else 'N/A'}")
            ...
            logger.error(f"Error loading history from {self.history_file_path}: {type(e).__name__}: {e}")
    ...
    def append_string(self, command: str):
        ...
        if command and (
            ...
            logger.debug(f"Adding new command to history: {command[:50]}{'...' if len(command) > 50 else ''}")
            self._loaded_strings.append(command)
            # Save immediately to disk
            self._save_single_entry(command)
        else:
            logger.debug(f"Skipping duplicate or empty command: {command[:50] if command else '(empty)'}")
        ...
        try:
            ...
            if not parent_dir.exists():
                ...
                try:
                    ...
                    logger.error(f"Failed to create directory {parent_dir}: {dir_error}")
                    logger.error(f"Directory permissions: Parent of {parent_dir} is writable: {parent_dir.parent.exists() and os.access(parent_dir.parent, os.W_OK)}")
                    raise
            ...
            with open(self.history_file_path, "a", encoding="utf-8") as f:
                ...
                logger.debug(f"Successfully wrote command to history")
            ...
            logger.error("Try setting QX_HISTORY_FILE environment variable to a writable location")
        except OSError as e:
            logger.error(f"OS error when saving to history: {e}")
            logger.error(f"This might be due to disk full, read-only filesystem, or network issues")
        except Exception as e:
            logger.error(f"Unexpected error saving command to history: {type(e).__name__}: {e}")
            logger.error(f"History file path: {self.history_file_path}")
