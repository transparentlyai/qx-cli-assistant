...
class AgentInvocationService:
    ...
    async def _get_or_create_mcp_manager(self) -> MCPManager:
        ...
        if self._mcp_manager is None:
            ...
            # Use BufferedConsole for non-interactive output
            buffered_console = BufferedConsole()
            # Don't need a parent task group for lightweight usage
            self._mcp_manager = MCPManager(
                console=buffered_console, parent_task_group=None
            )
            # Load available MCP configurations
            self._mcp_manager.load_mcp_configs()
    ...
    async def invoke_agent(
        ...
        try:
            ...
            task = asyncio.create_task(
                self._execute_agent(
                    agent_config, prompt, use_message_history, invocation_id
                )
            )
    ...
    async def _execute_agent(
        ...
        try:
            ...
            llm_agent = await initialize_agent_with_mcp(
                mcp_manager=mcp_manager, agent_config=agent_config
            )
            ...
            if use_message_history and response_messages:
                self._agent_manager.save_agent_message_history(
                    agent_config.name, response_messages
                )
    ...
    async def invoke_agent_batch(
        ...
        tasks = [invoke_with_semaphore(prompt, i) for i, prompt in enumerate(prompts)]
