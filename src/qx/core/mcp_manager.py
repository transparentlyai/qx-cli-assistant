...
class MCPManager:
    def __init__(self, console: Any, parent_task_group: Optional[anyio.abc.TaskGroup]):
        ...
        self._tools_by_server: Dict[str, List[Tuple[Callable, Dict[str, Any], Type[BaseModel]]]] = {}
    ...
    def load_mcp_configs(self):
        ...
        for path in config_paths:
            if path.exists() and path.is_file():
                try:
                    ...
                    Console().print(f"[yellow]Warning:[/yellow] Failed to load MCP configurations from {path}: {e}")
            else:
        ...
        if not self._available_servers:
            ...
            logger.info(f"Available MCP servers: {list(self._available_servers.keys())}")
    ...
    async def connect_server(self, server_name: str) -> bool:
        ...
        if self._parent_task_group is None:
            from rich.console import Console
            Console().print("[red]Error:[/red] MCPManager not initialized with a parent task group. Cannot connect.")
            return False

        if server_name not in self._available_servers:
            Console().print(f"[red]Error:[/red] MCP server '{server_name}' not found in configurations.")
            return False

        if server_name in self._active_tasks: # Check _active_tasks for running session
            Console().print(f"[blue]Info:[/blue] MCP server '{server_name}' is already connected or connecting.")
            return True
        ...
        async def _manage_session_task(*, task_status: anyio.abc.TaskStatus):
            ...
            try:
                ...
                if task_object is None: # Should not happen in normal asyncio execution
                    raise RuntimeError("Could not retrieve current asyncio task object.")
            except RuntimeError as e:
                logger.critical(f"MCP task for '{server_name}': Failed to get task handle: {e}", exc_info=True)
            ...
            try:
                ...
                for mcp_tool in response.tools:
                    try:
                        qx_tool_func, qx_tool_schema, qx_tool_input_model = self._convert_mcp_tool_to_qx_format(session, mcp_tool)
                        new_tools.append((qx_tool_func, qx_tool_schema, qx_tool_input_model))
                        ...
                        Console().print(f"[red]Error:[/red] Failed to register tool '{mcp_tool.name}' from '{server_name}': {e}")
                
                self._tools_by_server[server_name] = new_tools # Store tools for this server

                Console().print(f"[green]Success:[/green] Connected to MCP server '{server_name}' and loaded {len(new_tools)} tools.")
                ...
                Console().print(f"[red]Error:[/red] MCP session for '{server_name}' encountered an error: {e}")
                ...
                if self._active_tasks.get(server_name) is task_object: # Remove only if it's the current task
        ...
        try:
            ...
            Console().print(f"[red]Error:[/red] Failed to start MCP session for '{server_name}': {e}")
    ...
    async def disconnect_server(self, server_name: str) -> bool:
        ...
        if server_name not in self._active_tasks:
            Console().print(f"[blue]Info:[/blue] MCP server '{server_name}' is not currently connected or already disconnected.")
            return False
        ...
        Console().print(f"[green]Success:[/green] Disconnection requested for MCP server '{server_name}'.")
        return True
    ...
    def get_active_tools(self) -> List[Tuple[Callable, Dict[str, Any], Type[BaseModel]]]:
        ...
        logger.debug(f"Returning {len(all_tools)} active tools from {len(self._tools_by_server)} servers.")
        return all_tools

    @staticmethod
    def _convert_mcp_tool_to_qx_format(session: ClientSession, mcp_tool: Any) -> Tuple[Callable, Dict[str, Any], Type[BaseModel]]:
        """
        ...
        for prop_name, prop_details in input_schema.get("properties", {}).items():
            ...
            if prop_name in input_schema.get("required", []):
                ...
                default_value_from_schema = prop_details.get("default") # Field handles None if key absent
                fields[prop_name] = (field_type, Field(default=default_value_from_schema, description=description))
        ...
        async def wrapper_func(console: Any, args: BaseModel) -> Any:
            ...
            logger.debug(f"Calling MCP tool '{tool_name}' with args: {args.model_dump_json(indent=2)}")
            try:
        ...
        logger.debug(f"Converted MCP tool '{tool_name}' to QX format. OpenAI schema: {json.dumps(openai_tool_schema, indent=2)}")
