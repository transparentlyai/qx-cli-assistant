...
class MCPManager:
        ...
        for path in config_paths:
            if path.exists() and path.is_file():
                try:
                    ...
                    self.console.print(f"[yellow]Warning:[/yellow] Failed to load MCP configurations from {path}: {e}")
            else:
    ...
    async def connect_server(self, server_name: str) -> bool:
        ...
        if self._parent_task_group is None:
            self.console.print("[red]Error:[/red] MCPManager not initialized with a parent task group. Cannot connect.")
            return False

        if server_name not in self._available_servers:
            self.console.print(f"[red]Error:[/red] MCP server '{server_name}' not found in configurations.")
            return False

        if server_name in self._active_tasks: # Check _active_tasks for running session
            self.console.print(f"[blue]Info:[/blue] MCP server '{server_name}' is already connected or connecting.")
            return True
            ...
            try:
                ...
                for mcp_tool in response.tools:
                    try:
                        ...
                        self.console.print(f"[red]Error:[/red] Failed to register tool '{mcp_tool.name}' from '{server_name}': {e}")
                
                self._tools_by_server[server_name] = new_tools # Store tools for this server

                self.console.print(f"[green]Success:[/green] Connected to MCP server '{server_name}' and loaded {len(new_tools)} tools.")
                ...
                self.console.print(f"[red]Error:[/red] MCP session for '{server_name}' encountered an error: {e}")
        ...
        try:
            ...
            self.console.print(f"[red]Error:[/red] Failed to start MCP session for '{server_name}': {e}")
