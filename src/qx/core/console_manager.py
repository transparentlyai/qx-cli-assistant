...
class ConsoleManager:
    ...
    def stop(self, timeout: float = 5.0) -> None:
        ...
        if self._consumer_thread:
            ...
            if self._consumer_thread.is_alive():
                logger.warning("ConsoleManager consumer thread did not stop within timeout")
    ...
    def _consumer_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.error(f"Error in console manager consumer loop: {e}", exc_info=True)
    ...
    def request_approval_blocking(
        ...
        if not self._running:
            ...
            import asyncio
            from qx.core.user_prompts import request_confirmation
            return asyncio.run(request_confirmation(
                prompt_message, console, content_to_display,
                current_value_for_modification, default_choice_key
            ))
    ...
    def request_choice_blocking(
        ...
        if not self._running:
            ...
            import asyncio
            from qx.core.user_prompts import get_user_choice_from_options_async
            return asyncio.run(get_user_choice_from_options_async(
