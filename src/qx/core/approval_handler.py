...
def set_global_agent_context(
    agent_name: str, agent_color: Optional[str] = None
) -> None:
        ...
        if agent_name:
            try:
                ...
                bordered_md = BorderedMarkdown(
                    rich_text, border_style=f"dim {color}", background_color="#080808"
                )

                if self.use_console_manager and self._console_manager:
                    self._console_manager.print(
                        bordered_md, console=self.console, markup=True, end=""
                    )
                else:
    ...
    async def request_approval(
        ...
        if agent_name:
            ...
            if content_to_display:
                if self.use_console_manager and self._console_manager:
                    self._console_manager.print(
                        content_to_display, console=self.console
                    )
                else:
        ...
        styled_prompt = prompt_message.replace("[primary]", "[bright_blue]").replace(
            "[highlight]", "[magenta]"
        )
        ...
        if self.use_console_manager and self._console_manager:
            self._console_manager.print(
                full_styled_prompt, console=self.console, end=""
            )
        else:
